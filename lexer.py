...
class Lexer:
    ...
    def advance(self):
        if not self.reachedEnd(): self.forward = self.forward + 1
    ...
    def scanTokens(self):
        ...
        while not self.reachedEnd():
            match self.peek():
                ...
                case '!': self.consumeToken(TokenType.BANG_EQUAL) if self.isMultiOperator() else self.consumeToken(TokenType.BANG)
                case '=': self.consumeToken(TokenType.EQUAL_EQUAL) if self.isMultiOperator() else self.consumeToken(TokenType.EQUAL)
                case '<': self.consumeToken(TokenType.LESS_EQUAL) if self.isMultiOperator() else self.consumeToken(TokenType.LESS)
                case '>': self.consumeToken(TokenType.GREATER_EQUAL) if self.isMultiOperator() else self.consumeToken(TokenType.GREATER)
