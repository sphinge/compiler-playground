...
# ezcFilePath = sys.argv[1] #TODO: handle wrong intput
...
class Lexer:
    ...
    def __init__(self, pSymbolTable: SymbolTable = {}):
    ...
    def generateTokens(self, input_string: str):
    ...
    def handleNumber(self):
        ...
        if self.lookahead() == ".":
            ...
            self.consumeToken(
                TokenType.INT,
                int(self.currentLine[self.lexemeBegin : self.forward + 1]),
            )
    ...
    def scanTokens(self):
        while not self.reachedEnd():
            match self.peek():
                case "[":
                    self.consumeToken(TokenType.LEFT_BRACK)
                case "]":
                    self.consumeToken(TokenType.RIGHT_BRACK)
                case "(":
                ...
                case "!":
                    self.consumeToken(TokenType.BANG_EQUAL) if self.isFollowedBy(
                        "="
                    ) else self.consumeToken(TokenType.BANG)
                case "=":
                    self.consumeToken(TokenType.EQUAL_EQUAL) if self.isFollowedBy(
                        "="
                    ) else self.consumeToken(TokenType.EQUAL)
                case "<":
                    self.consumeToken(TokenType.LESS_EQUAL) if self.isFollowedBy(
                        "="
                    ) else self.consumeToken(TokenType.LESS)
                case ">":
                    self.consumeToken(TokenType.GREATER_EQUAL) if self.isFollowedBy(
                        "="
                    ) else self.consumeToken(TokenType.GREATER)
                case "/":
                    self.consumeToken(
                        TokenType.SLASH
