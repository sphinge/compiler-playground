...
class Label:
    ...
    @staticmethod
    def reset_number_of_labels() -> None:
        Label.number_of_labels = 0
...
def ASSIGNMENT_synth(node):
    ...
    node.code = (
        ...
        + node.children[0].code
        + [f'{node.children[1].lexval} = {node.children[3].res["name"]};']
    )
...
def PRINTSTMT_synth(node):
    ...
    node.code = (
        expression.code
        + node.managed_labels["EXPR_next"].set()
        + [f'printf("{c_type_id}", {expression.res["name"]});']
    )
...
def EXPR_bool_highest_add_gotos(node):
    ...
    if node.true_label and node.false_label:
        node.code += [
            f'if ({node.res["name"]}) {"{"} goto {node.true_label.get()};{"}"}',
            f'if (!({node.res["name"]})) {"{"} goto {node.false_label.get()};{"}"}',
        ]
...
def EXPR_bool_synth(node):
    ...
    if exprx.code == [] and exprx.res == None:
        ...
        node.code = (
            ...
            + exprx_code
            + [
                f'{node.res["code"]} = {elevel1.res["name"]} {operand} {exprx.res["name"]}; '
            ]
        )
...
def VARORCALL_synth(node):
    ...
    if not funcallx.res:
        ...
        node.res = Temp(node.type)
        node.code = funcallx.code + [
            f'{node.res["code"]} = {variablename}{funcallx.res}; \n'
        ]
